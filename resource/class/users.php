...
//<a class='w3-hover-blue' href='index.php?q=inheritcourse' target='_new'>Inherit Subject</a>;
	...
	public function addstud($fname,$sex){
		...
		if (mysqli_num_rows($p) == 1) {
			...
			/*Student can't be added if there exist one of his course scoresheet submitted then such semester is concluded*/ 
			
			$check_if_course_forwarded = mysqli_query($link,"SELECT id FROM course WHERE lv_id='$level' AND sem_id='$semester' AND dpt_id='$dpt_id'");

			while(list($cv_id) = mysqli_fetch_row($check_if_course_forwarded)){
				$in_check_if_course_forwarded = mysqli_query($link,"SELECT * FROM flag WHERE course_id='$cv_id' AND scoresheet_save_flag=1");
			}

		
			if (mysqli_num_rows($in_check_if_course_forwarded) == 0) {
